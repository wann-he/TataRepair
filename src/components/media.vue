<template>
    <el-scrollbar>
        ...
        <div class="content-box path-box">
            ...
            <div style="padding: 10px" v-show="checked1 || checked2 || checked3">
                <div class="right-top-icon-container" style="margin-top: 35px">
                    <el-button type="primary" plain @click="selectVideo" :disabled="allDisabled"
                        v-show="checked1 || checked2 || checked3">
                        <el-icon>
                            <Film />
                        </el-icon>&nbsp;选择视频
                    </el-button>
                    <el-button type="primary" plain @click="selectAudio" :disabled="allDisabled" v-show="checked5">
                        <el-icon>
                            <Mic />
                        </el-icon>&nbsp;选择音频
                    </el-button>
            ...
            <el-space :size="'default'" spacer="" style="margin-top: 30px" direction="vertical" alignment="flex-start">
                <el-row :gutter="24">
                    <el-col :span="24" v-show="checked1">
                        <el-form-item label="输出文件夹路径">
                            <el-col :span="20">
                                <el-input v-model="out_path" placeholder="输出文件夹路径" :disabled="true"
                                    @click="selectOutDir" size="default" clearable>
                                    <template #append>
                                        <el-button @click="selectOutDir" type="default" style="width: 115px"
                                            :disabled="allDisabled">点击此处选择
                                        </el-button>
                                    </template>
                                </el-input>
                            </el-col>
                ...
                <el-row :gutter="24">
                    ...
                    <el-col :span="24" v-show="checked3 || checked2">
                        <el-form-item label="GPU加速" v-show="checked2">
                            <el-switch v-model="useGpu" :disabled="allDisabled" />
                        </el-form-item>
            ...
            <el-form-item label="倍速" v-show="checked2">
                <el-slider v-model="speed" show-input :step="0.1" :min="0.1" :max="5" :disabled="allDisabled" />
            </el-form-item>
            ...
            <el-tabs v-model="activeName" class="demo-tabs" @tab-click="handleClick"
                ...
                <el-tab-pane label="任务列表" name="first">
                    ...
                    <el-row :gutter="24">
                        <el-col :span="24" v-show="checked1 || checked2 || checked3">
                            <keep-alive>
            ...
            <el-card v-show="checked4">
                <el-form-item label="Base文件夹" v-show="checked4">
                    <el-input v-model="out_path" placeholder="需求描述中可用'${path}'来指代" :disabled="true"
                        @click="selectOutDir" size="default" clearable>
                        <template #append>
                            <el-button @click="selectOutDir" type="default" style="width: 115px"
                                :disabled="allDisabled">点击此处选择
                ...
                <el-form-item label="选择模型">
                    <el-select v-model="qwen_model" placeholder="选择模型" size="default">
                        <el-option v-for="item in QWNE_OPTIONAL_MODELS" :key="item" :label="item" :value="item" />
                    </el-select>
                </el-form-item>

                <el-form-item label="需求描述">
                    <el-input v-model="question" placeholder="描述媒体文件处理需求，注意一定是用单行ffmpeg命令可以处理的需求" autosize :rows="2" type="textarea" clearable>
                    </el-input>
                </el-form-item>
                <el-form-item>
                    <div class="text-right">
                        <el-button @click="chat_chat()" :loading="chatBtnLoading" type="primary" link>生成命令行
                        </el-button>
                    </div>
                </el-form-item>

                <el-form-item label="生成命令">
                    <el-input v-model="answer" placeholder="命令行可以自行修改，确保命令行以 ffmpeg 开头，否则无法执行" clearable />
                </el-form-item>
                ...
                <el-form-item>
                    <div class="text-right">
                        <el-button :disabled="!executable" @click="execute()" :loading="executeBtnLoading" type="primary"
                            link>执 行
                        </el-button>
                        <el-button @click="execute_2()" :loading="executeBtnLoading" type="primary" link>
                            执行 _2
                        </el-button>
                    </div>
                </el-form-item>
                <el-form-item label="执行日志">
                    <el-input v-model="execute_logs" placeholder="" :rows="9" type="textarea" :disabled="true"
                        clearable>
                    </el-input>
                </el-form-item>
            </el-card>
...
import { appCacheDir, appDataDir, desktopDir } from '@tauri-apps/api/path'
import type { TabsPaneContext } from 'element-plus'
import { ElMessage } from 'element-plus'
import { convertFileSrc } from '@tauri-apps/api/core'
import { open as shellopen } from "@tauri-apps/plugin-shell";
import { MultipleVal, QwenModelVal } from "../script/constants";
import { convert, executeCustomCommand, executeCustomCommand_2, video2Audio } from "../script/media";
...
const suffixVal = ref<'_copy' | '_tata'>('_tata')
...
const execute_2 = () => {
    executeBtnLoading.value = true;
    executeCustomCommand_2(answer.value)
        .then((res) => {
            execute_logs.value = res.data;
            ElMessage({
                message: '执行完成',
...
async function selectVideo() {
    const isMultiple = checked1.value || checked2.value || checked3.value ? true : false;
    const selected = await open({
        ...
        filters: [
            {
                name: "视频文件",
                extensions: ['mp4', '3gp', 'avi', 'mov', 'm4a', 'webm', 'm4v', 'rmvb', 'rm', 'rm', 'dat', 'mkv', 'flv', 'vob', 'wmv']
            }
        ],
    })
    if (selected) {
        ...
        const result = await convert(
            ...
            .then((res) => {
                ...
                if (duration >= 60) {
                    ...
                    durationStr = `${Math.floor(duration)} 秒`;
                }
                if (res.rcode == 0) {
                    const message = `批量格式转换完成, 已处理${videos.value.length}个视频, 耗时${durationStr}`;
                    if (res.failedNum && res.failedNum > 0) {
                        sendNotify(`${message}, 失败${res.failedNum}个`);
                    } else {
                        sendNotify(message);
                    }
                } else {
